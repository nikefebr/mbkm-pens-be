<?php
    ...
    if ($search == null) {
        ...
        WHERE R.STUDENT_ID = $studentId AND R.STATUS_KEGIATAN='Kegiatan Terdaftar' 
        ORDER BY R.ID";
        ...
        R.NAMA_KEGIATAN
        FROM MBKM_PROGRAM P RIGHT JOIN MBKM_REGISTRATION R ON P.ID = MBKM_PROGRAM_ID 
        WHERE (R.STUDENT_ID = $studentId AND R.STATUS_KEGIATAN='Kegiatan Terdaftar') AND (R.NAMA_KEGIATAN LIKE '%$search%' 
        OR P.PROGRAM_NAME LIKE '%$search%' OR R.MITRA_NAME LIKE '%$search%' OR P.TAHUN_AJARAN LIKE '%$search%' 
        OR P.SEMESTER LIKE '%$search%' OR R.STATUS LIKE '%$search%')
        ORDER BY R.ID";
