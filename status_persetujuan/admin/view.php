<?php
    ...
    if ($search == null) {
        $query = 
        "SELECT P.ID, P.PROGRAM_NAME,P.SEMESTER, P.TAHUN_AJARAN, R.ID, R.STUDENT_ID, R.MBKM_PROGRAM_ID,
        ...
        R.DATE_END, R.STATUS_KEGIATAN, R.LOGBOOK,R.NAMA_KEGIATAN, M.NRP, M.NAMA
        FROM MBKM_PROGRAM P 
        ...
        FROM MBKM_PROGRAM P 
            RIGHT JOIN MBKM_REGISTRATION R ON P.ID = MBKM_PROGRAM_ID
            LEFT JOIN MAHASISWA M ON R.STUDENT_ID = M.NRP
        WHERE R.NAMA_KEGIATAN LIKE '%$search%' OR P.PROGRAM_NAME LIKE '%$search%' OR R.MITRA_NAME LIKE '%$search%' OR 
